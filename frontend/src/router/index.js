...
const businessRoutes = [
  ...
  { path: 'device/alarm', component: () => import('@/views/device/AlarmView.vue'), meta: { title: '安灯接警', roles: ['device', 'manager'] } },
  { path: 'production/andon-report', component: () => import('@/views/dashboard/operator/OperatorAndonView.vue'), meta: { title: '安灯上报', roleKey: 'operator' } },
...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.meta.roles && !to.meta.roles.includes(userStore.roleKey)) {
    next(home())
    return
  }
  next()
