  ...
  <div class="dev-dash">
    ...
    <div class="dd-row dd-row--charts">
      <div class="panel">
        ...
        <BoardChart v-if="equipments.length" :option="statusDonut" class="panel__chart" />
      ...
      <div class="panel">
        ...
        <BoardChart v-if="equipments.length" :option="workshopBar" class="panel__chart" />
        <div v-else class="panel__empty">暂无数据</div>
      </div>
    </div>
    ...
    <!-- 设备状态墙 -->
    <div class="panel">
      <div class="panel__title">设备状态墙</div>
      <div class="wall">
        <div v-for="e in equipments" :key="e.equipmentId" class="wall__chip" :class="'is-'+e.status.toLowerCase()">
...
const workshopBar = computed(() => {
  ...
  return {
    ...
    xAxis: { type: 'category', data: names, axisLine: { lineStyle: { color: '#dcdfe6' } }, axisTick: { show: false }, axisLabel: { color: '#606266', fontSize: 12 } },
    yAxis: { type: 'value', minInterval: 1, splitLine: { lineStyle: { color: '#f0f2f5' } }, axisLabel: { color: '#909399' } },
    series: [{
      type: 'bar', data: names.map((n) => map[n]), barWidth: '46%',
