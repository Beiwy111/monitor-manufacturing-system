  ...
  <div class="after-workbench">
    ...
    <main class="workbench-grid">
      ...
      <section class="main-stage">
        ...
        <div v-if="analyzing || triaging || bulkTriaging" class="scan-state">
          <div class="scan-line"><i /></div>
          <strong>{{ currentFlowSteps[currentFlowStep] }}</strong>
          <span>{{
            bulkTriaging
              ? `正在处理 ${bulkProgress.done}/${bulkProgress.total} 张售后工单`
              : triaging
                ? '正在理解客户诉求并核对 MES 证据'
                : '正在穿透生产、质量、设备与物料记录'
          }}</span>
...
const currentFlowSteps = computed(() => (triaging.value || bulkTriaging.value) ? triageSteps : analyzeSteps)
const currentFlowStep = computed(() => analyzeStep.value)
let flowTimer, graphAnimationFrame, traceResizeObserver, traceDragStartX = 0, traceDragStartScroll = 0, traceDragMoved = false

function startFlowTimer(steps, interval = 520) {
  clearInterval(flowTimer)
  analyzeStep.value = 0
  flowTimer = setInterval(() => {
    analyzeStep.value = Math.min(analyzeStep.value + 1, steps.length - 1)
  }, interval)
}
...
async function triageAllCases() {
  ...
  startFlowTimer(triageSteps, 520)
...
async function runTriage(force = false) {
  ...
  startFlowTimer(triageSteps, 520)
  try {
    const result = await fetchAfterSalesTriage(selectedCase.value.caseNo, force)
    triageMap.value = { ...triageMap.value, [selectedCase.value.caseNo]: result }
    triage.value = result
  } catch (e) {
...
async function runAnalysis(force = false) {
  ...
  analysis.value = null
  startFlowTimer(analyzeSteps, 620)
  try {
    const [result, trace] = await Promise.all([
      fetchRcaAnalysis(selectedCase.value.caseNo, force),
      fetchTraceDetail(selectedCase.value.caseNo).catch(() => null)
    ])
