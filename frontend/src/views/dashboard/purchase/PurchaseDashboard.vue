<template>
  <div class="pdb">
    <div class="pdb-panel">
      ...
      <div class="pdb-divider" />

      <!-- 物料需求与库存明细 -->
      <section class="pdb-section pdb-section--last">
        <div class="section-head">
          <span class="section-title">物料需求与库存明细</span>
          <span class="section-sub">{{ materialDetailHint }}</span>
          <span class="section-link" @click="$router.push('/purchase/demand')">进入需求工作台</span>
        </div>
        <el-table :data="materialDetailRows" border stripe size="small" v-loading="loading" style="width:100%"
          empty-text="暂无物料需求，请点击「刷新采购清单」">
          <el-table-column label="物料" min-width="200">
            <template #default="{ row }">
              <div class="material-cell">
                <img v-if="matImg(row)" class="material-thumb" :src="matImg(row)" :alt="row.materialName" />
                <div v-else class="material-thumb material-thumb--fallback">{{ matInitial(row) }}</div>
                <div class="material-cell__text">
          ...
          <el-table-column prop="supplierName" label="默认供应商" width="130" show-overflow-tooltip>
            <template #default="{ row }">
          ...
          <el-table-column prop="requiredQuantity" label="总需求" width="80" align="right">
            <template #default="{ row }"><strong>{{ fmtQty(row.requiredQuantity) }}</strong></template>
          </el-table-column>
          <el-table-column prop="stockQuantity" label="实际库存" width="88" align="right">
            <template #default="{ row }">
              <span :class="Number(row.stockQuantity) >= Number(row.requiredQuantity) ? 'stock-ok' : 'stock-low'">{{ fmtQty(row.stockQuantity) }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="onPurchaseQuantity" label="在途" width="72" align="right" />
          <el-table-column prop="shortageQuantity" label="净缺料" width="80" align="right">
            <template #default="{ row }">
              <strong v-if="Number(row.shortageQuantity ?? row.netShortage) > 0" style="color:#cf1322">{{ fmtQty(row.shortageQuantity ?? row.netShortage) }}</strong>
              <span v-else style="color:#52c41a">充足</span>
            </template>
          </el-table-column>
          <el-table-column prop="suggestedPurchaseQuantity" label="建议采购" width="88" align="right">
            <template #default="{ row }">
              <strong style="color:#d46b08">{{ fmtQty(row.suggestedPurchaseQuantity ?? row.netShortage) }}</strong>
            </template>
          </el-table-column>
          <el-table-column prop="priority" label="优先级" width="76" align="center">
            <template #default="{ row }">
              <el-tag v-if="row.priority" :type="row.priority === 1 ? 'danger' : row.priority === 2 ? 'warning' : 'info'" size="small">
...
/** 汇总所有订单 BOM 物料（缺料计算为空时的兜底展示） */
const aggregatedBomMaterials = computed(() => {
  const map = new Map()
  for (const od of orderDemands.value) {
    for (const m of od.materials || []) {
      const key = m.materialId || m.materialCode
      if (!key) continue
      ...
      if (!existing) {
        map.set(key, {
          ...
          onPurchaseQuantity: Number(m.onPurchaseQuantity || 0),
          netShortage: Number(m.netShortage || 0)
        })
      } else {
        existing.requiredQuantity += Number(m.requiredQuantity || 0)
        existing.netShortage += Number(m.netShortage || 0)
      }
    }
  }
  return [...map.values()].sort((a, b) => b.netShortage - a.netShortage)
})

const materialDetailRows = computed(() => {
  const all = purchasableReqs.value
  if (all.length) return all
  return aggregatedBomMaterials.value
})

const materialDetailHint = computed(() => {
  if (purchasableReqs.value.length) return '含缺料与备库可采购物料 · 库存充足也可下单'
  if (aggregatedBomMaterials.value.length) return '按 BOM 实时展开 · 库存充足时显示「充足」'
  return '展示物料需求、库存与建议采购量'
})
...
.pdb-divider { height: 1px; background: #f0f0f0; margin: 16px 0 0; }
