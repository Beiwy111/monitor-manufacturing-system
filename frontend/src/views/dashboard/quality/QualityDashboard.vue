<template>
  <div class="qd">
    <div class="qd-bar">
      <div class="qd-bar__title">
        <h2>质量管理驾驶舱</h2>
        <span>一次合格率 · 缺陷帕累托 · 不良品闭环</span>
      </div>
      <div class="qd-bar__meta">
        <span class="qd-updated">更新 {{ updatedText }}</span>
    ...
    <!-- 指标条（点击下钻到处理页） -->
    <div class="qd-strip">
      <div v-for="m in metrics" :key="m.key" class="strip-item" :class="{ 'strip-item--hero': m.hero, 'is-link': m.to }"
        @click="m.to && go(m.to)">
        <div class="strip-num" :class="m.cls">{{ m.num }}<i v-if="m.suffix">{{ m.suffix }}</i></div>
        <div class="strip-lab">{{ m.label }}<span v-if="m.to" class="strip-go">›</span></div>
      </div>
    </div>

    <!-- 第一行：帕累托 + 结论分布 -->
    <div class="qd-grid qd-grid--a">
      <section class="card">
        <header class="card__hd"><i class="tick tick--blue" />缺陷帕累托分析<span class="card__note">按缺陷类型频次排列，虚线为 80% 控制线</span></header>
        <BoardChart v-if="defectEntries.length" :option="paretoOption" class="chart chart--h" />
        <p v-else class="empty">暂无不良品数据</p>
      </section>

      <section class="card">
        <header class="card__hd"><i class="tick tick--blue" />质检结论分布</header>
        ...
        <p v-else class="empty">暂无质检数据</p>
      </section>
    </div>

    <!-- 第二行：处置闭环 + 待检任务 -->
    <div class="qd-grid qd-grid--b">
      <section class="card">
        <header class="card__hd"><i class="tick tick--amber" />不良品处置闭环</header>
        <template v-if="nc.length">
          <div class="flow">
            <div v-for="seg in dispositionSegs" :key="seg.key" class="flow__seg"
              :style="{ flexGrow: seg.value || 0.001, background: seg.color }" :title="`${seg.name} ${seg.value}`">
              <span v-if="seg.value">{{ seg.value }}</span>
            </div>
          </div>
          <div class="flow__legend">
            <span v-for="seg in dispositionSegs" :key="seg.key"><i :style="{ background: seg.color }" />{{ seg.name }} {{ seg.value }}</span>
          </div>
          <div class="method">
            ...
            <div v-for="m in methodRows" :key="m.key" class="method__row">
              ...
              <div class="method__track"><div class="method__fill" :style="{ width: pct(m.value, nc.length) + '%', background: m.color }" /></div>
      ...
      <section class="card">
        <header class="card__hd"><i class="tick tick--blue" />待检 / 复检任务<span class="card__act" @click="go('/quality/inspection')">去检验 ›</span></header>
        <el-table v-if="pendingTasks.length" :data="pendingTasks" size="small" max-height="252" class="clickable"
          @row-click="(row) => go(row.inspectionStatus === 'RECHECK_REQUIRED' ? '/quality/reinspect' : '/quality/inspection')">
        ...
        <p v-else class="empty empty--ok">✓ 无待检任务，检验进度已清零</p>
      </section>
    </div>

    <!-- 第三行：不良品台账 -->
    <section class="card">
      <header class="card__hd"><i class="tick tick--red" />不良品台账<span class="card__act" @click="go('/quality/defect')">去处置 ›</span></header>
      <el-table v-if="nc.length" :data="ncSorted" size="small" max-height="300" class="clickable"
        @row-click="() => go('/quality/defect')">
        ...
        <el-table-column label="严重度" width="82" align="center">
          <template #default="{ row }"><el-tag :type="sevType(row.severity)" size="small">{{ sevCn(row.severity) }}</el-tag></template>
        </el-table-column>
        ...
        <el-table-column label="处置状态" width="90" align="center">
          <template #default="{ row }"><el-tag :type="handleType(row.handleStatus)" size="small">{{ handleCn(row.handleStatus) }}</el-tag></template>
        </el-table-column>
        <el-table-column prop="registeredAt" label="登记时间" width="160" />
      </el-table>
      <p v-else class="empty">暂无不良品记录</p>
    </section>
  </div>
</template>
...
const CATEGORY_CN = { SEMI_FINISHED: '半成品', FINISHED_PRODUCT: '成品' }
...
// 合格率按“单据结论”口径：QUALIFIED / 已判定（有结论）
...
const metrics = computed(() => [
  { key: 'pending', num: counts.value.pending, label: '待检任务', to: '/quality/inspection' },
  { key: 'rate', num: passRate.value, suffix: '%', label: '一次交检合格率', hero: true, to: '/quality/records' },
  { key: 'recheck', num: counts.value.recheck, label: '需复检', cls: counts.value.recheck ? 'warn' : '', to: '/quality/reinspect' },
  { key: 'ncOpen', num: ncOpen.value, label: '不良品待处置', cls: ncOpen.value ? 'danger' : '', to: '/quality/defect' },
  { key: 'ncClosed', num: `${ncClosed.value} / ${nc.value.length}`, label: '不良品已闭环', to: '/quality/defect' }
])
...
<style scoped>
.qd { padding: 14px 18px; background: #eef1f5; min-height: 100%; }

.qd-bar { display: flex; align-items: flex-end; justify-content: space-between; margin-bottom: 12px; }
.qd-bar__title h2 { margin: 0; font-size: 19px; font-weight: 700; color: #1a2434; }
.qd-bar__title span { font-size: 12px; color: #8a94a6; }
.qd-bar__meta { display: flex; align-items: center; gap: 10px; }
.qd-updated { font-size: 12px; color: #a0a6b0; }

/* 指标条 */
.qd-strip { display: flex; align-items: stretch; background: #fff; border: 1px solid #e4e8ee; border-radius: 6px; padding: 4px 0; margin-bottom: 12px; }
.strip-item { flex: 1; text-align: center; padding: 10px 6px; border-radius: 6px; transition: background .15s; }
.strip-item + .strip-item { border-left: 1px solid #edf0f4; }
.strip-item.is-link { cursor: pointer; }
.strip-item.is-link:hover { background: #f5f8ff; }
.strip-num { font-size: 24px; font-weight: 700; color: #1a2434; line-height: 1.1; }
.strip-num i { font-size: 14px; font-weight: 600; color: #8a94a6; font-style: normal; margin-left: 1px; }
.strip-num.warn { color: #d48806; }
.strip-num.danger { color: #cf3838; }
.strip-lab { margin-top: 5px; font-size: 12px; color: #8a94a6; }
.strip-go { color: #c0c6d0; margin-left: 3px; }
.strip-item.is-link:hover .strip-go { color: #1668dc; }
.strip-item--hero { flex: 1.3; }
.strip-item--hero .strip-num { font-size: 30px; color: #1668dc; }

/* 卡片 */
.card { background: #fff; border: 1px solid #e4e8ee; border-radius: 6px; padding: 12px 14px; margin-bottom: 12px; }
.card__hd { display: flex; align-items: center; font-size: 14px; font-weight: 600; color: #1a2434; margin-bottom: 10px; }
.card__note { margin-left: auto; font-size: 12px; font-weight: 400; color: #a0a6b0; }
.card__act { margin-left: auto; font-size: 12px; font-weight: 500; color: #1668dc; cursor: pointer; }
.card__act:hover { text-decoration: underline; }
.clickable :deep(.el-table__row) { cursor: pointer; }
.tick { width: 3px; height: 13px; border-radius: 2px; margin-right: 7px; }
.tick--blue { background: #1668dc; }
.tick--amber { background: #d48806; }
.tick--red { background: #cf3838; }
.chart--h { height: 264px; }
.empty { text-align: center; color: #a0a6b0; padding: 56px 0; font-size: 13px; margin: 0; }
.empty--ok { color: #52a838; }

.qd-grid { display: grid; gap: 12px; }
.qd-grid--a { grid-template-columns: 1.55fr 1fr; }
.qd-grid--b { grid-template-columns: 1fr 1.55fr; }

/* 结论分布 */
.conc { display: flex; align-items: center; gap: 8px; }
.conc__ring { width: 190px; height: 200px; flex: 0 0 190px; }
.conc__legend { flex: 1; list-style: none; margin: 0; padding: 0; }
.conc__legend li { display: flex; align-items: center; padding: 7px 4px; border-bottom: 1px dashed #eef1f5; font-size: 13px; }
.conc__legend li:last-child { border-bottom: none; }
.dot { width: 9px; height: 9px; border-radius: 2px; margin-right: 8px; flex: 0 0 9px; }
.conc__name { color: #4a5568; }
.conc__val { margin-left: auto; font-weight: 700; color: #1a2434; }
.conc__pct { width: 44px; text-align: right; color: #a0a6b0; font-size: 12px; }

/* 处置闭环 */
.flow { display: flex; height: 30px; border-radius: 4px; overflow: hidden; background: #f0f2f5; }
.flow__seg { display: flex; align-items: center; justify-content: center; color: #fff; font-size: 13px; font-weight: 600; min-width: 0; transition: flex-grow .3s; }
.flow__legend { display: flex; gap: 16px; margin: 8px 0 4px; font-size: 12px; color: #6b7280; }
.flow__legend span { display: flex; align-items: center; }
.flow__legend i { width: 9px; height: 9px; border-radius: 2px; margin-right: 5px; }
.method { margin-top: 12px; border-top: 1px solid #eef1f5; padding-top: 10px; }
.method__title { font-size: 12px; color: #8a94a6; margin-bottom: 8px; }
.method__row { display: flex; align-items: center; margin-bottom: 8px; font-size: 13px; }
.method__name { width: 62px; color: #4a5568; }
.method__track { flex: 1; height: 8px; background: #f0f2f5; border-radius: 4px; overflow: hidden; margin: 0 8px; }
.method__fill { height: 100%; border-radius: 4px; }
.method__val { width: 20px; text-align: right; font-weight: 600; color: #1a2434; }

@media (max-width: 1180px) {
  .qd-strip { flex-wrap: wrap; }
  .qd-grid--a, .qd-grid--b { grid-template-columns: 1fr; }
}
</style>
