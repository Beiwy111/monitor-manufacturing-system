<template>
  <div class="ruoyi-page inspection-page">
    ...
    <div class="qc-split-layout">
      ...
      <div class="qc-split-layout__side">
        ...
        <div class="qc-section">
          ...
          <button v-if="isFinishedMode" class="vision-entry" type="button" @click="visionVisible=true">
            <span class="vision-entry__icon">AI</span>
            <span><b>AI 外观检测</b><small>上传电脑显示器图片，检测屏幕划痕区域</small></span>
          </button>
    ...
    <AppearanceInspectionDialog v-model="visionVisible" :inspection="selected" />
  </div>
</template>
<!-- SCRIPT_PLACEHOLDER -->
...
import AppearanceInspectionDialog from '@/components/quality/AppearanceInspectionDialog.vue'
...
<style scoped>
.vision-entry{width:100%;display:flex;align-items:center;gap:10px;margin:10px 0 12px;padding:10px 12px;border:1px solid #91caff;border-radius:8px;background:linear-gradient(135deg,#eff6ff,#e6f4ff);color:#164c8c;cursor:pointer;text-align:left;transition:.18s}.vision-entry:hover{border-color:#409eff;box-shadow:0 4px 12px #409eff20;transform:translateY(-1px)}.vision-entry__icon{display:grid;place-items:center;width:34px;height:34px;border-radius:9px;background:#1677ff;color:#fff;font-size:12px;font-weight:800}.vision-entry b,.vision-entry small{display:block}.vision-entry b{font-size:13px}.vision-entry small{margin-top:3px;color:#6483a8;font-size:10px}
.sampling-hint {
  margin-left: 10px;
  font-size: 12px;
  color: #909399;
