<template>
  <ModulePageShell>
    ...
    <ModuleListDetailLayout
      :has-detail="!!selected"
      detail-width="420px"
      empty-description="点击左侧质检单查看详情和操作"
    >
      <!-- 左侧列表 -->
      <template #list>
        <div class="list-toolbar">
          <el-input v-model="keyword" placeholder="质检单/批次/物料" clearable size="small" style="width:190px" />
          <el-select v-model="categoryFilter" clearable placeholder="分类" size="small" style="width:108px;margin-left:6px">
      ...
      <template #detail>
        ...
        <!-- 操作区 -->
        <ModulePanelSection v-if="selected.inspectionStatus!=='CLOSED'">
          <div class="panel-title">质检操作</div>
        ...
        <ModulePanelSection v-else><el-tag type="info">质检单已关闭</el-tag></ModulePanelSection>

        <!-- 关联不良品 -->
        <ModulePanelSection v-if="detail?.nonconformingList?.length">
          <div class="panel-title">关联不良品</div>
...
// 从路由 meta 获取品类约束（semi 路由 = SEMI_FINISHED，fp 路由 = FINISHED_PRODUCT）
const routeCategory = computed(() => route.meta?.category || '')
const pageTitle = computed(() => routeCategory.value === 'SEMI_FINISHED' ? '半成品质检' : routeCategory.value === 'FINISHED_PRODUCT' ? '成品质检' : '全部质检')
...
async function ensureMaterials() {
  if (Object.keys(materialMap.value).length) return
  try {
    const res = await request.get('/material/material/list')
    const arr = res?.data ?? res ?? []
    const m = {}
    for (const x of arr) m[x.materialId] = x.materialName
    materialMap.value = m
  } catch { /* 忽略，退化为空 */ }
}
...
const kpiCards = [
  ...
  { key: 'SEMI_FINISHED',    kpiKey: 'semiFinished',    label: '半成品',    cls: 'semi' },
...
const filtered = computed(() => {
  ...
  if (tabActive.value !== 'all') {
    if (['SEMI_FINISHED', 'FINISHED_PRODUCT'].includes(tabActive.value)) {
