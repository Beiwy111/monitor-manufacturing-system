<template>
  <ModulePageShell>
    ...
    <!-- 一键生成采购单弹窗（按供应商分组预览） -->
    <el-dialog v-model="generateVisible" title="生成采购订单" width="620px" @open="onGenerateOpen">
      <div v-if="supplierGroups.length === 0" style="color:#909399;text-align:center;padding:20px">
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="generating" @click="doGenerate">
          确认生成 {{ supplierGroups.length }} 张采购单
        </el-button>
      </template>
    </el-dialog>
  </ModulePageShell>
...
async function doGenerate() {
  ...
  try {
    // 构建 supplierOverrides：每个供应商组的到货日期
    const overrides = {}
    for (const grp of supplierGroups.value) {
      if (grp.expectedArrivalDate) {
        overrides[String(grp.supplierId)] = {
          expectedArrivalDate: grp.expectedArrivalDate
        }
      }
    }
    const orders = await generatePurchaseOrder({
