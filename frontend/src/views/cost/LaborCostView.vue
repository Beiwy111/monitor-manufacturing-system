<template>
  <div class="cost-sub-page">
    <div class="page-header">
      <span class="page-title">人工成本</span>
    ...
    <div class="summary-bar">
      ...
      <div class="sum-item highlight-green"><span class="sum-val">¥ {{ fmtAmt(sumOf('laborCost')) }}</span><span class="sum-lbl">人工成本合计</span></div>
      <div class="sum-item"><span class="sum-val">¥ {{ fmtAmt(sumOf('totalCost')) }}</span><span class="sum-lbl">总成本合计</span></div>
      <div class="sum-item"><span class="sum-val">{{ pct('laborCost') }}%</span><span class="sum-lbl">人工占比</span></div>
    </div>
    <el-table :data="filtered" border stripe size="small" style="width:100%" v-loading="loading">
      ...
      <el-table-column label="人工成本(¥)" width="115" align="right">
        <template #default="{ row }"><b style="color:#67c23a">{{ row.laborCost ?? '-' }}</b></template>
      </el-table-column>
      <el-table-column prop="totalCost"     label="总成本(¥)"  width="110" align="right" />
      <el-table-column label="人工占比" width="90" align="center">
        <template #default="{ row }">
          {{ Number(row.totalCost) > 0 ? Math.round(Number(row.laborCost)/Number(row.totalCost)*100) + '%' : '-' }}
        </template>
      </el-table-column>
      <el-table-column prop="costReason"    label="成本原因"   min-width="160" show-overflow-tooltip />
...
.sum-item.highlight-green { border-top: 3px solid #67c23a; }
