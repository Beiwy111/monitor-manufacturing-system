...
async function audit(pass) {
  ...
  const ok = await mes.auditOrder(selected.value.id, pass, userStore.username, userStore.roleKey)

  if (!ok) {

    ElMessage.warning('当前订单状态不允许审核')

    return

  }

  ElMessage.success(pass ? '审核通过，订单已进入计划员待办' : '订单已作废')
