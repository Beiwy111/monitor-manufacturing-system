...
const pendingEquipments = computed(() =>
  equipments.value.filter(e =>
    e.isProductionWorkshop !== false &&
    (e.status === 'FAULT' || e.status === 'MAINTAINING' || Number(e.openAlarmCount) > 0)
  )
)
...
async function doResolve() {
  acting.value = true
  try {
    await resolveAlarm({ alarmId: resolveForm.alarmId, remark: '手动解除', operator: operator.value })
...
onMounted(() => {
  loadAll()
  alarmPollTimer = setInterval(loadAll, 30000)
})
onUnmounted(() => { if (alarmPollTimer) clearInterval(alarmPollTimer) })
</script>
