...
public class EquipmentServiceImpl implements EquipmentService {
    ...
    public Map<String, Object> equipmentKpi() {
        ...
        kpi.put("normal", all.stream().filter(e -> EQ_IDLE.equals(safe(e.getStatus())) || EQ_RUNNING.equals(safe(e.getStatus()))).count());
        kpi.put("fault", all.stream().filter(e -> EQ_FAULT.equals(safe(e.getStatus()))).count());
        kpi.put("maintaining", all.stream().filter(e -> EQ_MAINTAINING.equals(safe(e.getStatus()))).count());
        kpi.put("scrapped", all.stream().filter(e -> EQ_SCRAPPED.equals(safe(e.getStatus()))).count());
    ...
    public List<Map<String, Object>> calcHealthList() {
        ...
        return equipments.stream().map(eq -> {
            ...
            // 近7天报警列表（供详情抽屉展示）
            Map<Long, User> userMap = userMap();
            List<Map<String, Object>> alarmList7d = alarms.stream()
                .filter(a -> eqId.equals(a.getEquipmentId()))
                .filter(a -> a.getReportedAt() != null && a.getReportedAt().isAfter(since7d))
                .sorted((a, b) -> b.getReportedAt().compareTo(a.getReportedAt()))
                .map(a -> alarmView(a, equipmentMap(), userMap))
                .collect(Collectors.toList());

            // 最近5条维保记录
            List<Map<String, Object>> maintList = records.stream()
                .filter(r -> eqId.equals(r.getEquipmentId()))
                .sorted((a, b) -> {
                ...
                .limit(5)
                .map(r -> maintenanceView(r, equipmentMap(), userMap))
                .collect(Collectors.toList());
            ...
            result.put("faultCount30d",  faultCount30d);
            ...
            result.put("advice",         genAdvice(score, code, alarm7d, daysSinceMaint, faultCount30d));
            result.put("alarmList7d",    alarmList7d);
            result.put("maintList",      maintList);
            return result;
        }).sorted((a, b) -> Integer.compare(
            (Integer) a.get("healthScore"), (Integer) b.get("healthScore")
        )).collect(Collectors.toList());
    }
