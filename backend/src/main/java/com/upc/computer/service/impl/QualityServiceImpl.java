...
public class QualityServiceImpl implements QualityService {
    ...
    public List<QualityInspectionItem> generateDefaultItems(Long inspectionId) {
        ...
        String type = "FINISHED_PRODUCT".equals(insp.getInspectionCategory())
                ? "FINISHED_PRODUCT" : insp.getInspectionType();
        List<QualityInspectionItem> defaults = buildDefaultItems(type, inspectionId, ds);
        LocalDateTime now = LocalDateTime.now();
    ...
    private List<QualityInspectionItem> buildDefaultItems(String type, Long inspectionId, DisplaySpec ds) {
        if (type == null) type = "";
    ...
    private String categoryCn(String s) {
        ...
        return switch (s) {
            case "SEMI_FINISHED" -> "半成品质检"; case "FINISHED_PRODUCT" -> "成品质检"; default -> s;
        };
    }
