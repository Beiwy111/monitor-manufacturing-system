...
public interface QualityInspectionMapper {
    ...
    @Select("SELECT COUNT(*) AS total, SUM(CASE WHEN inspection_status='PENDING' THEN 1 ELSE 0 END) AS pending, SUM(CASE WHEN inspection_status='PASSED' THEN 1 ELSE 0 END) AS passed, SUM(CASE WHEN inspection_status='FAILED' THEN 1 ELSE 0 END) AS failed, SUM(CASE WHEN inspection_status='RECHECK_REQUIRED' THEN 1 ELSE 0 END) AS recheck, SUM(CASE WHEN inspection_category='SEMI_FINISHED' THEN 1 ELSE 0 END) AS semiFinished, SUM(CASE WHEN inspection_category='RAW_MATERIAL' THEN 1 ELSE 0 END) AS rawMaterial, SUM(CASE WHEN inspection_category='FINISHED_PRODUCT' THEN 1 ELSE 0 END) AS finishedProduct, SUM(CASE WHEN inspection_status IN ('PENDING','RECHECK_REQUIRED') AND inspection_category='FINISHED_PRODUCT' THEN 1 ELSE 0 END) AS pendingFinishedProduct, SUM(CASE WHEN inspection_status IN ('PENDING','RECHECK_REQUIRED') AND inspection_category='RAW_MATERIAL' THEN 1 ELSE 0 END) AS pendingRawMaterial FROM quality_inspection")
