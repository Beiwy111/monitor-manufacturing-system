...
public class PurchaseController {
    ...
    // 订单需求总览：订单数量、成品库存、需采购量（活数据）
    @GetMapping("/workbench/orderDemands")
    public Result<List<Map<String, Object>>> orderDemands() {
        return Result.success(purchaseService.listOrderDemandOverview());
    }
    ...
    // 保存草稿采购单（修改明细数量、单价、供应商信息）
    @PostMapping("/order/saveDraft")
    public Result<PurchaseOrder> savePurchaseOrderDraft(@RequestBody UpdatePurchaseOrderDraftRequest request) {
        return Result.success(purchaseService.savePurchaseOrderDraft(request));
    }
